...
def test_audio_worker_mute_timing_accuracy():
    ...
    worker = AudioWorker(settings=settings, audio_source=MagicMock())
    ...
    with patch("winsound.Beep"):
        ...
        worker._audio_callback(silent_data, None)
...
def test_audio_worker_whisper_tolerance():
    ...
    worker._audio_callback(noise_data, None)
    worker._audio_callback(noise_data, None)
...
def test_audio_worker_recovery_from_silence():
    ...
    worker._audio_callback(loud_data, None)
